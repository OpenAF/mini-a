(function() {
  ...
  exports.testMetadata = function() {
    ...
    ow.test.assert(isDef(metadata.fileOps), true, "Should include fileOps metadata")
    ow.test.assert(isDef(metadata.fileModify), true, "Should include fileModify metadata")
    ow.test.assert(isDef(metadata.mathOps), true, "Should include mathOps metadata")
    ow.test.assert(isDef(metadata.timeOps), true, "Should include timeOps metadata")
    ...
    ow.test.assert(methods.indexOf("fileOps") >= 0, true, "Should include fileOps method")
    ow.test.assert(methods.indexOf("fileModify") >= 0, true, "Should include fileModify method")
    ow.test.assert(methods.indexOf("mathOps") >= 0, true, "Should include mathOps method")
    ow.test.assert(methods.indexOf("timeOps") >= 0, true, "Should include timeOps method")
  }
  ...
  exports.testMathOpsCalculate = function() {
    ...
    // Test addition
    var result1 = tool.mathOps({ operation: "calculate", op: "add", values: [1, 2, 3, 4] })
    ow.test.assert(result1.result === 10, true, "Should add values correctly")

    // Test subtraction
    var result2 = tool.mathOps({ operation: "calculate", op: "subtract", values: [10, 3, 2] })
    ow.test.assert(result2.result === 5, true, "Should subtract values correctly")

    // Test multiplication
    var result3 = tool.mathOps({ operation: "calculate", op: "multiply", values: [2, 3, 4] })
    ow.test.assert(result3.result === 24, true, "Should multiply values correctly")

    // Test division
    var result4 = tool.mathOps({ operation: "calculate", op: "divide", values: [20, 2, 5] })
    ow.test.assert(result4.result === 2, true, "Should divide values correctly")

    // Test power
    var result5 = tool.mathOps({ operation: "calculate", op: "power", values: [2, 3] })
    ow.test.assert(result5.result === 8, true, "Should calculate power correctly")

    // Test square root
    var result6 = tool.mathOps({ operation: "calculate", op: "sqrt", values: [16] })
    ow.test.assert(result6.result === 4, true, "Should calculate square root correctly")

    // Test absolute value
    var result7 = tool.mathOps({ operation: "calculate", op: "abs", values: [-5] })
    ow.test.assert(result7.result === 5, true, "Should calculate absolute value correctly")

    // Test rounding
    var result8 = tool.mathOps({ operation: "calculate", op: "round", values: [3.7] })
    ow.test.assert(result8.result === 4, true, "Should round values correctly")

    // Test precision
    var result9 = tool.mathOps({ operation: "calculate", op: "divide", values: [10, 3], precision: 2 })
    ow.test.assert(result9.result === 3.33, true, "Should respect precision parameter")

    // Test empty values
    var result10 = tool.mathOps({ operation: "calculate", op: "add", values: [] })
    ow.test.assert(isString(result10) && result10.indexOf("[ERROR]") === 0, true, "Should error with empty values")

    // Test unknown operation
    var result11 = tool.mathOps({ operation: "calculate", op: "unknown", values: [1, 2] })
  ...
  exports.testMathOpsStatistics = function() {
    ...
    var result1 = tool.mathOps({ operation: "statistics", values: [1, 2, 3, 4, 5] })
    ...
    var result2 = tool.mathOps({ operation: "statistics", values: [1, 2, 3, 4] })
    ow.test.assert(result2.median === 2.5, true, "Should calculate median correctly for even count")

    // Test specific metrics
    var result3 = tool.mathOps({ operation: "statistics", values: [10, 20, 30], metrics: ["mean", "max"] })
    ...
    // Test empty values
    var result4 = tool.mathOps({ operation: "statistics", values: [] })
  ...
  exports.testMathOpsConvert = function() {
    ...
    var result1 = tool.mathOps({ operation: "convert-unit", value: 1, fromUnit: "km", toUnit: "m" })
    ow.test.assert(result1.result === 1000, true, "Should convert km to m correctly")

    var result2 = tool.mathOps({ operation: "convert-unit", value: 100, fromUnit: "cm", toUnit: "m" })
    ow.test.assert(result2.result === 1, true, "Should convert cm to m correctly")

    // Test weight conversion
    var result3 = tool.mathOps({ operation: "convert-unit", value: 1, fromUnit: "kg", toUnit: "g" })
    ow.test.assert(result3.result === 1000, true, "Should convert kg to g correctly")

    // Test volume conversion
    var result4 = tool.mathOps({ operation: "convert-unit", value: 1, fromUnit: "l", toUnit: "ml" })
    ow.test.assert(result4.result === 1000, true, "Should convert l to ml correctly")

    // Test precision
    var result5 = tool.mathOps({ operation: "convert-unit", value: 1, fromUnit: "mi", toUnit: "km", precision: 2 })
    ow.test.assert(result5.result === 1.61, true, "Should respect precision in conversion")

    // Test unknown unit
    var result6 = tool.mathOps({ operation: "convert-unit", value: 1, fromUnit: "unknown", toUnit: "m" })
    ow.test.assert(isString(result6) && result6.indexOf("[ERROR]") === 0, true, "Should error with unknown unit")

    // Test convert alias
    var result7 = tool.mathOps({ operation: "convert", value: 1, fromUnit: "km", toUnit: "m" })
  ...
  exports.testMathOpsRandom = function() {
    ...
    var result1 = tool.mathOps({ operation: "random", type: "integer", min: 1, max: 10, seed: 12345 })
    ...
    var result2 = tool.mathOps({ operation: "random", type: "integer", min: 0, max: 100 })
    ow.test.assert(result2.value >= 0 && result2.value <= 100, true, "Random integer should be in range")

    // Test random sequence
    var result3 = tool.mathOps({ operation: "random", type: "sequence", start: 1, end: 5, count: 3, seed: 67890 })
    ...
    // Test random choice
    var result4 = tool.mathOps({ operation: "random", type: "choice", items: ["a", "b", "c"], count: 2, seed: 11111 })
    ...
    var result5 = tool.mathOps({ operation: "random", type: "choice", items: [1, 2, 3, 4, 5], count: 3, unique: true, seed: 22222 })
    ...
    // Test random boolean
    var result6 = tool.mathOps({ operation: "random", type: "boolean", count: 10, probabilityTrue: 0.5, seed: 33333 })
    ...
    // Test random hex
    var result7 = tool.mathOps({ operation: "random", type: "hex", length: 16, seed: 44444 })
  ...
  exports.testTimeOpsCurrentTime = function() {
    ...
    // Test current time
    var result1 = tool.timeOps({ operation: "current-time" })
    ...
    var result2 = tool.timeOps({ operation: "current-time", timezone: "America/New_York" })
    ow.test.assert(result2.timezone === "America/New_York", true, "Should use specified timezone")

    // Test with custom format
    var result3 = tool.timeOps({ operation: "current-time", format: "yyyy-MM-dd" })
    ...
    // Test current alias
    var result4 = tool.timeOps({ operation: "current" })
  ...
  exports.testTimeOpsConvert = function() {
    ...
    var result3 = tool.timeOps({ operation: "convert" })
  ...
  exports.testTimeOpsSleep = function() {
    ...
    var result1 = tool.timeOps({ operation: "sleep", milliseconds: 100 })
    ...
    // Test negative sleep
    var result2 = tool.timeOps({ operation: "sleep", milliseconds: -1 })
  ...
  exports.testMathOpsUnknownOperation = function() {
    ...
    var result = tool.mathOps({ operation: "unknown-op" })
