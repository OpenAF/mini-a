...
- **`usememory`** (string): JSSLON/SLON definition for the OpenAF channel that backs Mini-A's medium/long-term memory. When provided, Mini-A spins up a single dummy MCP tool named `memory` that lets the LLM remember (`action=remember` → `$ch.set`), recall (`action=recall` → `$ch.get`), or forget (`action=forget` → `$ch.unset`) entries keyed by `key`. Example: `usememory="(type: 'file', options: (file: '/tmp/mini-a-memory.json'))"`.
- **`memoryro`** (boolean, default: false): Marks the memory tool as read-only. When true, `action=recall` works normally but attempts to remember or forget entries are blocked.
...
| `memory` | `remembers`, `recalls`, `forgets`, `errors`, `read_only_blocks` | Memory tool usage counters driven by the `usememory` dummy MCP channel. Tracks successful store/get/delete operations, blocked attempts when `memoryro=true`, and error responses. |
