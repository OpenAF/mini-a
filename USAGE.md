...
- **`promptprofile`** (string): Control system prompt verbosity. `balanced` (the default) omits worked examples and the step-by-step "how to call MCP tools" walkthroughs, includes full plan-schema guidance only while a plan is first being generated, and trims MCP/utils tool-schema descriptions to their essential clause — this is a meaningfully leaner prompt than earlier releases while keeping all functional guidance. `minimal` goes further: no examples, no tool-schema descriptions for optional parameters, terse planning reminders only, and at most 8 skills listed. `verbose` restores the full worked examples, complete tool-schema descriptions, and detailed planning guidance at all times — use it if you hit unexpected tool-calling mistakes and want the richer guidance back. When unset, Mini-A defaults to `minimal` in chatbot mode, `verbose` with `debug=true` outside chatbot mode, and `balanced` otherwise.
...
- **`wikisourceinline`** (boolean, default: `false`): Also append the rendered URL into search results' `description`/`summary` text (`[sourceUrl: <url>]`), for models that attend to prose more reliably than to a sibling JSON key.
- **`OAF_MINI_A_WIKI_SOURCE_URL`** (environment variable, optional): Environment equivalent of `wikisourceurl`; an explicit runtime `wikisourceurl` value takes precedence.
...
- **`wikisearchscanbudget`** (number, default: `1000`) / **`wikisearchscanmaxms`** (number, default: `15000`): Cap how many pages, and how many milliseconds, `wiki search`'s scan-fallback path (triggered by `forceScan`, `regex`, a scoped `path`, or an unavailable Lucene index) may spend reading pages one at a time. The budget is shared across a search and its mount fan-out — it is not reset per mount. When either cap is hit, the search returns early with whatever results were already found; direct JS callers (not the serialized MCP tool text) can check `.truncated`/`.scanned`/`.scanBudget` on the returned array to see whether it stopped short.
- **`wikisearchcache`** (boolean, default: `true` for `s3`/`http`/`es` backends, `false` for `fs`/archive) / **`wikisearchcachettlms`** (number, default: `15000`) / **`wikisearchcachemaxsize`** (number, default: `500`): Cache scan-fallback `backend.read()` results per `MiniAWikiManager` instance for the given TTL (capped to `wikisearchcachemaxsize` entries), so repeated searches against slow S3/HTTP backends don't re-fetch unchanged pages; `fs`/archive backends default to uncached since a local read has no latency to amortize and caching would only add staleness for pages edited outside the manager. Set `wikisearchcache` explicitly to override the default either way. On write or delete, the whole per-instance cache is invalidated and lazily rebuilt (not a per-page unset) — coarser than per-key invalidation, but writes/deletes are rare relative to reads.
- **`wikisearchparallel`** (boolean, default: `false`): Parallelize the scan-fallback path's `backend.read()` calls via `pForEach`, opt-in. A prior `pForEach`-based read path elsewhere in the wiki manager deadlocked OpenAF's shared thread pool under repeated same-JVM runs, and that failure's root cause was never fully isolated (see `docs/WIKI.md` and the code comment above the `wikisearchparallel` branch in `mini-a-wiki.js`) — leave this off unless you've validated it against your own workload with two consecutive full test-suite runs in one JVM.
...
**Recommended wiki ops sequence (agent or MCP):** `context` → `search` → `read` (with `section=` for long pages) → `write`/`lint`. The `context` op returns a compact overview in <500 tokens — use it once to orient before any search. `search` returns `{path, title, description}` by default (no full content); add `contextLines>0` for snippets. Lucene-backed results also include a numeric `score`, the native Lucene relevance score, retained as an input to future hybrid ranking; scan fallback results omit it. `wiki op="retrieve" query="..."` composes this discovery-and-bounded-read path into a compact cited evidence packet. Its `maxCandidates`, `maxInspected`, `maxBytes`, and optional `maxGraphExpansion` bounds are reported with actual usage; use `expandGraph=true` only when backlinks/graph relations are relevant. `list withMeta=true` returns metadata for all pages in one call. Reads to mounted wikis use the `@name/path.md` syntax; writes are always to the primary wiki.
...
Phase 2 can be enabled explicitly with `historyvm=true contextvirtualization=true`. It extends the same canonical journal with stable typed handles, parent/child metadata, deterministic L0-L4 representation levels, hierarchical summaries, coarse-to-fine internal retrieval, and bounded line/section/JSON-path reads. Derived views are generated and cached only when requested; exact content remains canonical. These Phase 2A/2B foundations do not yet replace the Phase 1 projection or adaptive selection policy, so enabling them alone does not claim additional input-token savings.
...
**Auto-promotion and staleness**: Mini-A finalizes memory on every run exit, including a missing final answer or an exception. When `memorypromote` is non-empty, it promotes configured session sections to global storage. Keyed records such as validated tool contracts are replaced by key so the latest shape wins; other records use refresh-or-append. After promotion, if `memorystaledays > 0`, a sweep marks old global entries `stale=true`. Stale entries remain visible but are deprioritized and evicted first when compaction runs.
