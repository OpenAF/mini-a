...
MiniA.prototype._ingestPlanKnowledge = function(args, knowledgeText) {
  ...
  var planFilePath = this._planFileTarget
  if (!isString(planFilePath) || planFilePath.length === 0) {
    if (isObject(args) && isString(args.plan)) planFilePath = args.plan.trim()
    if ((!isString(planFilePath) || planFilePath.length === 0) && isObject(args) && isString(args.planfile)) {
      planFilePath = args.planfile.trim()
    }
  }
  var planFromFile = ""
  if (planFilePath.length > 0) {
    if (io.fileExists(planFilePath) && io.fileInfo(planFilePath).isFile) {
      try {
        ...
        info.origin = "file"
        this._planFileTarget = planFilePath
      } catch (pfErr) {
...
MiniA.prototype._resolvePlanFileTarget = function(args) {
  var planPath = ""
  if (isObject(args)) {
    var explicitPlan = isString(args.plan) ? args.plan.trim() : ""
    var legacyPlanFile = isString(args.planfile) ? args.planfile.trim() : ""

    if (explicitPlan.length > 0) planPath = explicitPlan
    else if (legacyPlanFile.length > 0) planPath = legacyPlanFile

    if (planPath.length > 0) {
      args.plan = planPath
      if (legacyPlanFile.length === 0) args.planfile = planPath
    } else {
      args.plan = ""
    }
  }
...
MiniA.prototype._convertPlanToJsonPhases = function(plan) {
  if (!isObject(plan) || !isArray(plan.phases)) return []

  var phases = []
  var planTitle = isString(plan.title) ? plan.title.trim() : ""
  var dependencies = isArray(plan.dependencies) ? plan.dependencies.slice() : []
  var notes = isArray(plan.notes) ? plan.notes.slice() : []

  for (var p = 0; p < plan.phases.length; p++) {
    var phase = plan.phases[p]
    if (!isObject(phase)) continue
    var phaseTitle = isString(phase.title) && phase.title.trim().length > 0
      ? phase.title.trim()
      : `Phase ${p + 1}`
    var phaseTasks = isArray(phase.tasks) ? phase.tasks : []
    var tasks = []
    for (var t = 0; t < phaseTasks.length; t++) {
      var task = phaseTasks[t]
      if (!isObject(task)) continue
      var label = isString(task.label) && task.label.trim().length > 0 ? task.label.trim() : `(task ${t + 1})`
      tasks.push({
        label    : label,
    ...
    var entry = {
      phase : `Phase ${p + 1}`,
      title : phaseTitle,
      index : isNumber(phase.index) ? phase.index : p,
      tasks : tasks
    }

    if (planTitle.length > 0) entry.planTitle = planTitle
    if (p === 0 && dependencies.length > 0) entry.dependencies = dependencies
    if (p === 0 && notes.length > 0) entry.notes = notes
...
MiniA.prototype._renderMarkdownPlan = function(plan) {
  if (!isObject(plan)) return ""
  var lines = []
  var title = isString(plan.title) && plan.title.trim().length > 0 ? plan.title.trim() : "Generated Plan"
  lines.push(`# Plan: ${title}`)

  var phases = isArray(plan.phases) ? plan.phases : []
  for (var p = 0; p < phases.length; p++) {
    var phase = phases[p]
    if (!isObject(phase)) continue
    var phaseTitle = isString(phase.title) && phase.title.trim().length > 0 ? phase.title.trim() : `Phase ${p + 1}`
    lines.push("")
    lines.push(`## Phase ${p + 1}: ${phaseTitle}`)
    var phaseTasks = isArray(phase.tasks) ? phase.tasks : []
    for (var t = 0; t < phaseTasks.length; t++) {
      var task = phaseTasks[t]
      if (!isObject(task)) continue
      var label = isString(task.label) && task.label.trim().length > 0 ? task.label.trim() : `(task ${t + 1})`
      var checkbox = task.checked === true ? "x" : " "
      lines.push(`- [${checkbox}] ${label}`)
    }
  }

  if (isArray(plan.dependencies) && plan.dependencies.length > 0) {
    lines.push("")
    lines.push("## Dependencies")
    for (var d = 0; d < plan.dependencies.length; d++) {
      var dep = plan.dependencies[d]
      if (isString(dep) && dep.trim().length > 0) lines.push(`- ${dep.trim()}`)
    }
  }
  ...
  if (isArray(plan.notes) && plan.notes.length > 0) {
    ...
    for (var n = 0; n < plan.notes.length; n++) {
      var note = plan.notes[n]
      if (isString(note) && note.trim().length > 0) lines.push(`- ${note.trim()}`)
    }
  }
...
MiniA.prototype._persistExecutionPlanToFile = function(options) {
  var planPath = this._planFileTarget
  if (!isString(planPath) || planPath.length === 0) {
    var execution = this._executionPlan
    if (isObject(execution) && isString(execution.planFile) && execution.planFile.length > 0) {
      planPath = execution.planFile
    }
  }
  if (!isString(planPath) || planPath.length === 0) return
...
MiniA.prototype._initializeExecutionPlan = function(options) {
  ...
  this._persistExecutionPlanToFile({ plan: parsed, reason: "initialize" })
}
...
MiniA.prototype._syncExecutionPlanState = function(context) {
  ...
  this._handlePlanUpdate()
  this._persistExecutionPlanToFile({ plan: exec.parsed, reason: "sync" })
}
...
MiniA.prototype._runPlanningMode = function(options) {
  ...
  var planFileTarget = this._resolvePlanFileTarget(args)
  ...
  var parsedPlan = this._parseMarkdownPlan(planMarkdown, { goal: goalText })
  var finalAnswer = planMarkdown
  var finalFormat = "md"

  if (isString(planFileTarget) && planFileTarget.length > 0) {
    try {
      var contentToWrite = planMarkdown.endsWith("\n") ? planMarkdown : planMarkdown + "\n"
      io.writeFileString(planFileTarget, contentToWrite)
      this._planFileTarget = planFileTarget
      this.fnI("done", `Plan saved to ${planFileTarget}`)
    } catch (writeErr) {
      this.fnI("warn", `Failed to save plan to '${planFileTarget}': ${writeErr && writeErr.message ? writeErr.message : writeErr}`)
    }

    var planJson = this._convertPlanToJsonPhases(parsedPlan)
    finalAnswer = planJson
    finalFormat = "json"
  }
  ...
  var outputArgs = merge({}, args, { format: finalFormat })
  outputArgs.__format = finalFormat
  return this._processFinalAnswer(finalAnswer, outputArgs)
}
...
MiniA.prototype.init = function(args) {
  ...
  try {
    ...
    this._plannerInsights = {}
    this._planFileTarget = ""
    this._resolvePlanFileTarget(args)
